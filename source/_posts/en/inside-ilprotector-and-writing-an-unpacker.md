...
ILProtector can be considered as an obfuscator with not strong strength and good compatibility, but there is almost no information about this obfuscator online. However, there are unpackers released by CodeCracker and some unpackers improved based on his unpacker.
...
To avoid various attempts without significant meaning, let's see how the mentioned open-source project achieves unpacking. Let's first open this project in vs. (This provides a packaged project for download: [ILProtectorUnpacker by RexProg.7z](./ILProtectorUnpacker%20by%20RexProg.7z))
...
No wonder it was detected. The first instruction is jmp, and the Hook is too direct. Let's do something fancy and add 0xEB 0x00 in front of "jmp 0x06715AA8", which is equivalent to "jmp eip/rip+2".
...
With this method written, we'll write another new method to get the address of Target and Detour, first write JunkCode to Tagret, and then write the actual Jmp jump.
